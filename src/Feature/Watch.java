...
public final class Watch {
    ...
    public static void watch(final ActionsInputData command, final ArrayList<UsersInputData> users,
        ...
        if (LiveInfo.getInstance().getCurrentPage().getPageType().equals("see details")) {
            ...
            for (int k = 0; k < LiveInfo.getInstance().getCurrentUser().getPurchasedMovies().size(); k++) {
                if (LiveInfo.getInstance().getCurrentUser().getPurchasedMovies().get(k).getName().equals(listOfOneMovie.get(0).getName())) {
                    /**
                     * We modify the watched section of the user
                     */
                    LiveInfo.getInstance().getCurrentUser().getWatchedMovies().add(listOfOneMovie.get(0));
