...
public final class Purchase {
    ...
    public static void purchase(final ActionsInputData command, final ArrayList<UsersInputData> users,
        ...
        if (LiveInfo.getInstance().getCurrentPage().getPageType().equals("see details")) {
            ...
            if (LiveInfo.getInstance().getCurrentUser().getCredentials().getAccountType().equals("premium")) {
                if (LiveInfo.getInstance().getCurrentUser().getNumFreePremiumMovies() >= 1) {
                    LiveInfo.getInstance().getCurrentUser().setNumFreePremiumMovies(LiveInfo.getInstance().getCurrentUser().getNumFreePremiumMovies() - 1);
                    /**
                     * we add the film in the purchased section
                     */
                    LiveInfo.getInstance().getCurrentUser().getPurchasedMovies().add(listOfOneMovie.get(0));
                ...
                } else if (LiveInfo.getInstance().getCurrentUser().getTokensCount() >= 2) {
                ...
                if (LiveInfo.getInstance().getCurrentUser().getTokensCount() >= 2) {
                    LiveInfo.getInstance().getCurrentUser().setTokensCount(LiveInfo.getInstance().getCurrentUser().getTokensCount() - 2);
                    /**
                     * we add the film in the purchased section
                     */
