...
public final class Register {
     ...
     * @param output
     */
    public static void register(final ActionsInputData command,
        ...
        if (LiveInfo.getInstance().getCurrentPage().getPageType().equals("register")) {
            ...
            for (int i = 0; i < Database.getInstance().getDatabaseUsers().size(); i++) {
                if (Database.getInstance().getDatabaseUsers().get(i).getCredentials().getName().
                    ...
                    if (Database.getInstance().getDatabaseUsers().get(i).getCredentials()
                            .getPassword().equals(command.getCredentials().getPassword())) {
                        /**
                         * The user is already registered with these credentials,
                         * so he is redirected to the HomePageNon
                         */
            ...
            if (!sw) {
                /**
                 * We add a user with the new credentials and move on HomePage
                 */
